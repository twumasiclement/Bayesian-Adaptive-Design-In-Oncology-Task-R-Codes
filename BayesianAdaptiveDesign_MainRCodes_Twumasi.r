...
# Compute interim ORR
###############################################################
data_clean_6Months<- subset(data_clean, Visit=="6-Month")

responses <- data_clean_6Months$Response
...
###############################################################
# Predictive Probability of Success (PPoS) calculation
# Interim: 6 responses out of 9 patients
# Posterior: Beta(7,4)
###############################################################
...
Nmax <- 30
n_interim <- 9
n_future <- Nmax - n_interim


target_ORR <- 0.30
success_boundary <- 0.80
...
success_vec <- numeric(nsim)



for(i in 1:nsim){
  
  # draw true ORR from interim posterior
  theta_sim <- rbeta(1, post_a, post_b)
  
  # simulate future responses
  future_resp <- rbinom(1, size = n_future, prob = theta_sim)
  
  # Step 3: compute final posterior
  r_total <- 6 + future_resp
  
  post_a_final <- 1 + r_total
  post_b_final <- 1 + Nmax - r_total
  
  # compute posterior probability at final
  prob_final <- 1 - pbeta(target_ORR, post_a_final, post_b_final)
  
  # check success rule
  success_vec[i] <- (prob_final > success_boundary)
}
...
PPoS <- mean(success_vec)

PPoS

PPoS_hat <- mean(success_vec)

SE <- sqrt(PPoS_hat * (1 - PPoS_hat) / nsim)

CI_lower <- PPoS_hat - 1.96 * SE
CI_upper <- PPoS_hat + 1.96 * SE

c(PPoS_hat, CI_lower, CI_upper)
